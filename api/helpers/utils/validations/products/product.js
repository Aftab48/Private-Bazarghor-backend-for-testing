...
const createProductByAdmin = joi.object({
  ...
  productDescription: joi.string().allow("", null).max(5000),
  quantity: joi.number().integer().min(0).required(),
  price: joi.number().min(0).required(),
  category: joi.string().valid(...CATEGORIES_LIST),
  subcategory: joi
    .string()
    .valid(...SUBCATEGORIES_LIST)
    .allow("", null),
...
const updateProductByAdmin = joi
  .object({
...
const getProductsByAdmin = joi.object({
  page: joi.number().integer().min(1).default(1),
  limit: joi.number().integer().min(1).max(100).default(10),
  vendorId: joi.string().allow("", null),
  storeId: joi.string().allow("", null),
...
const getProductByIdByAdmin = joi.object({
  id: joi.string().required().messages({
    "string.empty": "Product ID is required",
  }),
});
